...
export function SmoothCursor({
  ...
  const [isDesktop, setIsDesktop] = useState(true); // 👈 for desktop check
    ...
    const smoothMouseMove = (e) => {
      ...
      if (magneticTarget) {
        ...
        currentPos = {
          x:
            currentPos.x +
            (magneticTarget.x - currentPos.x) * strength * 0.3,
          y:
            currentPos.y +
            (magneticTarget.y - currentPos.y) * strength * 0.3,
        };
      }
  ...
  // ❌ If disabled or hidden or mobile, do not render
  if (disabled || !isVisible || !isDesktop) return null;
